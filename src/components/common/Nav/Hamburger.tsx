...
export const HamburgerButton = forwardRef<HTMLButtonElement, { reveal: boolean; toggle: () => void; classname?:string }>(
  ({ reveal, toggle,classname }, ref) => {
    return (
      <button
        ref={ref}
        aria-label="Mobile Menu"
        className={`cursor-pointer block lg:hidden ${classname}`}
        onClick={(e) => {
          e.stopPropagation()
          toggle()
        }}
